...
# OUTRA DEP - COM CERTEZA VOTARIA EM bOLSONARO ###### OUTRA INDEP 2
...
print(base$P10C04)
library(memisc)
dados$microchip <- memisc::recode(as.numeric(dados$P1A05), 1 <- c(1,2), 0 <- c(3,4,5,99))
...
levels(bolsonaro)
levels(bolsonaro) <- c('Não tem certeza','Votaria com certeza')
levels(bolsonaro)
print(base$P9C01)
...
b$' Voto em Bolsonaro' <- b$bolsonaro
...
g + geom_jitter(height = 0.4, width = 0.4) + labs(title = "Aprovação à Bolsonaro e negacionismo",
                        ...
                        x = "Há Microchip na Vacina de Covid 19?",
